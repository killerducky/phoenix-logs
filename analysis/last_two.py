...
class LastTwo(LogAnalyzer):
    ...
    def ParseLog(self, log, log_id):
        ...
        for riichi in riichis:
            ...
            if len(types) != 2:
                if len(types) == 1:
                    if types[0] - types[0] % 10 == riichi_suit:
                        self.counts[key]["1 Type Same Suit"] += 1
                        self.counts["Total"]["1 Type Same Suit"] += 1
                    else:
                        self.counts[key]["1 Type Different Suit"] += 1
                        self.counts["Total"]["1 Type Different Suit"] += 1
                else:
    ...
    def PrintResults(self):
        waits = ["1-4", "2-5", "3-6", "4-7", "5-8", "6-9", "Other Suit", "1 Type Same Suit", "1 Type Different Suit", "2 Types", "3+ Types"]

        with open("./results/LastTwo.csv", "w") as c:
            c.write("Pattern,1-4,2-5,3-6,4-7,5-8,6-9,Other Suit,1 Type Same Suit,1 Type Different Suit,2 Types,3+ Types\n")
